<?php
 ...
 if ($ultimo=='seleccionaUniversal'){
  	...
  	if (isset($_REQUEST['firstArea'])) $_REQUEST['prefecture']=$_REQUEST['firstArea'];
...
if ($ultimo=='selecciona'){
		...
		if (isset($_REQUEST['zipCode']) && !empty($_REQUEST['zipCode'])){
			...
			$zipBean = $svcZips->obtienePorCodigo("JAPAN", $_REQUEST['zipCode']);
		...
		foreach ($beans as $bean){
		  ...
		  $arrBean["urlCompleta"]= $GLOBALS["dirWeb"]  .  "/shelters/info/" . "japan" . "/" .  $arrBean["urlEncoded"];
		  $arrBean["distanciaFormateada"]=  number_format($arrBean["distanceKm"] , 1) . " km";		
		  $arrBean["locacion"] =  $arrBean['locality'] . ", " . $arrBean['adminArea1'];
	...
	echo json_encode($exito) ;	
        ...
        $uploads_dir = '../../resources/images/shelterLogos/japan';

...
if ($ultimo=='selecciona'){
		...
		$beans=$svc->selTodos($nombreOParte, $state, $county, $latitude, $longitude, $distance, $specialBreedId, $desde, $cuantos);
		$cuenta=$svc->selTodosCuenta($nombreOParte, $state, $county, $latitude, $longitude, $distance, $specialBreedId, $desde, $cuantos); 
  	  ...
  	  $bean=new BreederUsa();
  	  $svc = new BreedersUsaSvcImpl();
  	  $bean->setId($_REQUEST['breederUsaId']);
  	  $bean->setName($_REQUEST['name']);
  	  $bean->setZip($_REQUEST['zip']);
  	  $bean->setUrl($_REQUEST['url']);
  	  $bean->setUrlEncoded($_REQUEST['urlEncoded']);
	...
	echo json_encode($exito) ;	
        ...
        $uploads_dir = '../../resources/images/breederLogos/usa';

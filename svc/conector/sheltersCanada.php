<?php
  ...
  require_once $_SERVER['DOCUMENT_ROOT'] . $GLOBALS['dirAplicacion'] . '/beans/ShelterCanada.php';
...
if ($ultimo=='selecciona'){
		...
		$beans=$svc->selTodos($nombreOParte, $province, $latitude, $longitude, $distance, $specialBreedId, $desde, $cuantos);
		$cuenta=$svc->selTodosCuenta($nombreOParte, $province, $latitude, $longitude, $distance, $specialBreedId, $desde, $cuantos); 
		...
		foreach ($beans as $bean){
		  ...
		  $arrBean['distanceKm']=$bean->getDistancia() * 0.621371 ;  // pasa de km a millas
  	  ...
  	  $bean=new ShelterCanada();
  	  $svc = new SheltersCanadaSvcImpl();
  	  $bean->setId($_REQUEST['shelterCanadaId']);
  	  $bean->setName($_REQUEST['name']);
  	  $bean->setZip($_REQUEST['zip']);
  	  $bean->setUrl($_REQUEST['url']);
  	  $bean->setUrlEncoded($_REQUEST['urlEncoded']);
	...
	$svc = new SheltersCanadaSvcImpl();
	$exito=$svc->borra($_REQUEST['shelterCanadaId']);
	echo json_encode($exito) ;	
	
  }else if ($ultimo=='subeLogo'){
   	    header("Content-Type: text/html; charset=utf-8");
        $uploads_dir = '../../resources/images/shelterLogos/canada';
